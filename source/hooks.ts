...
export const onBeforeBuild: BuildHook.onBeforeBuild = async function (
  ...
  console.log(result);
  debugger;
  const filepath = join(Editor.Project.path, "assets", "NewComponent.ts");
...
export const onBeforeCompressSettings: BuildHook.onBeforeCompressSettings =
  async function (options: ITaskOptions, result: IBuildResult) {
...
export const onAfterCompressSettings: BuildHook.onAfterCompressSettings =
  async function (options: ITaskOptions, result: IBuildResult) {
    // Todo some thing
